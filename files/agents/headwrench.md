...
4.  **Tool Compliance:** When invoking the `task` tool, strictly use the parameters: `subagent_type`, `prompt`, and `description`. Never use the `command` parameter. The required call shape must match this structure: `task(subagent_type="<agent name>", description="<short 3-5 word description>", prompt="<full instructions to the agent>")`.
